...
<div
  class="rounded-2xl border bg-white p-4 shadow-md hover:shadow-lg transition-shadow flex items-start gap-4"
>
  <!-- Logo -->
  ...
  <div class="flex flex-col">
    ...
    <p class="text-sm text-gray-600">{brand.address}</p>

    <!-- Badges -->
    <div class="mt-2 flex flex-wrap gap-2">
      <p class="font-semibold text-gray-600">Available for:
        {#if brand.pickup}
          <span
            class=" ml-2 rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-700"
            >Pickup</span
          >
        {/if}
        {#if brand.delivery}
