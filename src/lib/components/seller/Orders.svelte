<script lang="ts">
  import OrderButtons from "$lib/components/seller/OrderButtons.svelte";
  ...
  function confirm(orderId: string, order: any) {
    dispatch("confirmOrder", { orderId, order });
  }

  function decline(orderId: string, order: any) {
    dispatch("declineOrder", { orderId, order });
  }

  $: filteredOrders = pendingOrders.filter(order => order.is_accepted === "pending");
</script>

<section>
  <h2 class="mb-6 text-2xl font-bold">Active Orders</h2>
    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      {#each filteredOrders as order}
        <div class="relative rounded-2xl border border-gray-200 bg-white p-6 shadow-lg">
          ...
          <!-- Action buttons (always visible if pending) -->
          {#if order.is_accepted === "pending"}
            <div class="flex gap-2">
              <OrderButtons
                on:confirmOrder={() => confirm(order.uuid, order)}
                on:declineOrder={() => decline(order.uuid, order)}
              />
            </div>
          {/if}
        </div>
  ...
  {:else}
    <div class="text-gray-500 italic">No orders yet.</div>
  {/if}
</section>
