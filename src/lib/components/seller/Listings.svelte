...
<section>
	<div class="mb-6 flex w-full items-center">
		<button
			on:click={addListing}
			class="flex w-full transform cursor-pointer justify-center rounded-xl bg-gradient-to-r
             from-green-600 to-emerald-500 px-5 py-3 text-4xl font-semibold
		...
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
			{#each myDeals as deal (deal.id)}
				<div class="group relative transform transition hover:scale-[1.02]">
					<ListingCard {deal} on:edit={editListing} on:delete={deleteListing} />
				</div>
			{/each}
		</div>
	{:else}
